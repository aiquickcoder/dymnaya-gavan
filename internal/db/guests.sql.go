...
const getGuestSummary = `-- name: GetGuestSummary :one
with u_orders as (
...
type GetGuestSummaryRow struct {
	...
	LtvMonth     float64       `json:"ltv_month"`
	AvgScore     pgtype.Float8 `json:"avg_score"`
	FavouriteMix *string       `json:"favourite_mix"`
}
...
func (q *Queries) GetGuestSummary(ctx context.Context, id uuid.UUID) (GetGuestSummaryRow, error) {
	row := q.db.QueryRow(ctx, getGuestSummary, id)
	var i GetGuestSummaryRow
	err := row.Scan(
		&i.ID,
		&i.PhoneNumber,
		&i.CreatedAt,
...
const listGuestsByRestaurant = `-- name: ListGuestsByRestaurant :many
...
type ListGuestsByRestaurantRow struct {
	ID           uuid.UUID     `json:"id"`
	PhoneNumber  string        `json:"phone_number"`
	CreatedAt    time.Time     `json:"created_at"`
	Visits       int32         `json:"visits"`
...
func (q *Queries) ListGuestsByRestaurant(ctx context.Context, restaurantID uuid.UUID) ([]ListGuestsByRestaurantRow, error) {
	rows, err := q.db.Query(ctx, listGuestsByRestaurant, restaurantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListGuestsByRestaurantRow{}
	for rows.Next() {
		var i ListGuestsByRestaurantRow
		if err := rows.Scan(
			&i.ID,
			&i.PhoneNumber,
			&i.CreatedAt,
